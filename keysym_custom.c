...
/* Don't use the uint32_t types here, to save some space */
struct codepair {
    uint16_t keysym;
    uint16_t ucs;
};
...
/* return count of string`s bytes*/
size_t 
...
characters_num(UTF8 *string){
    size_t chars_num = 0;
    for (; *string; ++string) if ((*string & 0xC0) != 0x80) ++chars_num;    
    return chars_num;
}
...
keysym_to_utf8(KeySym keysym, UTF8 *buffer, size_t size){
    ...
    codepoint = keysym_to_utf32(keysym);
...
utf8_to_utf32(UTF8* utf8_string, size_t *str_lens){
    size_t byte_string_length = byte_len(utf8_string);
    *str_lens = characters_num(utf8_string);
